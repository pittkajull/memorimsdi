...
function SprocketHoles() {
  return (
    <div className="flex gap-4 px-4">
      {[...Array(50)].map((_, i) => (
        <div key={i} className="w-4 h-3 bg-black/80 border border-zinc-600/30 flex-shrink-0"></div>
      ))}
    </div>
  );
}
      ...
      <div className="bg-zinc-900/90 border border-zinc-700/40 py-3">
        ...
        <div className="relative overflow-hidden my-3">
          <div
            ...
            style={{ animation: `${isLeft ? "scrollLeft" : "scrollRight"} 60s linear infinite` }}
          >
            {[...images, ...images, ...images, ...images].map((src, i) => (
              <div key={i} className="relative flex-shrink-0 w-72 h-48 md:w-80 md:h-56 overflow-hidden">
                <img
                  src={src}
                  alt={`Memory ${i + 1}`}
